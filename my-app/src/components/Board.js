...
const Board = () => {
    ...
    return (
        <div>
            ...
            <button onClick={() => navigate('/write')}>글쓰기</button> {/* 글쓰기 버튼 클릭 시 '/write' 경로로 이동 */}
            <ul>
                {posts.map((post, index) => (
                    // posts 배열을 반복하여 각 게시글을 리스트 항목으로 렌더링합니다.
                    <li key={index} onClick={() => navigate(`/boardDetail/${post.id}`)}>
                        {/* 리스트 항목 클릭 시 해당 게시글의 상세 페이지로 이동합니다. */}
                        <h3>{post.title}</h3> {/* 게시글 제목을 h3 요소로 표시합니다. */}
                        <p>{post.content}</p> {/* 게시글 내용을 p 요소로 표시합니다. */}
                        <p>작성자: {post.writer}</p> {/* 게시글 작성자를 p 요소로 표시합니다. */}
                    </li>
                ))}
            </ul>
        </div>
